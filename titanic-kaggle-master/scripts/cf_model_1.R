...
set.seed(322)
fit = cforest(as.factor(Survived) ~ Sex + Age + FamSize + Title + Pclass + Fare, data=train.eng, controls=cforest_unbiased(ntree=2000, mtry=3))


Prediction <- predict(fit, test.eng, OOB=TRUE, type='response')
submit = data.frame(PassengerId = test.eng$PassengerId)
rownames(submit) = rownames(test.eng)
submit$Survived = Prediction
write.csv(submit, file = "cf_model_1.csv", row.names = FALSE)
